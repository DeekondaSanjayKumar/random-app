...
const LabelFilter = () => {
	...
	const [animalFilter, setAnimalFilter] = useState(animalData)
	


	const filterClass = (animalClasse) =>{
		const filterData = animalData.filter((item)=>
			item.class === animalClasse
		)
		setAnimalFilter(filterData)
			...
			<div data-testid="labels-wrapper-id" className="label-container">
				{animalClasses.map((animalClass) => (
					<div
						...
						className="label"
						key={animalClass}
						onClick={()=>filterClass(animalClass)}
					>
						{animalClass}
					</div>
				))}
